        ...
        for x in range(1, len(arr)):  # for each number in array input
            ...
            for j in range(x - 1, -1, -1):  # for range in everything to left of current
                if arr[j] > current:  # if current in sorted array is greater than current in unsorted
                    ...
                    arr[j + 1] = (
                        current  # current in sorted is smaller than current unsorted,placing current unsorted next to current sorted
                    )
                    break
                arr[j] = (
                    current  # assign place in sorted to current in correct index ( only runs if less than to replace value)
                )
        print(arr)
